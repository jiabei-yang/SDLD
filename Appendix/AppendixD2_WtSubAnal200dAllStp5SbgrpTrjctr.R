...
mean(subData.nomiss.wt200d.sbgrpWt.resPop.scaled$male)
1-mean(subData.nomiss.wt200d.sbgrpWt.resPop.scaled$male)

# mean((subData.nomiss.wt200d.sbgrpWt.resPop.scaled$male == 0) & (subData.nomiss.wt200d.sbgrpWt.resPop.scaled$time0_dob < final.tree.wt200d$splits[2, "index"]))
# mean((subData.nomiss.wt200d.sbgrpWt.resPop.scaled$male == 0) & (subData.nomiss.wt200d.sbgrpWt.resPop.scaled$time0_dob >= final.tree.wt200d$splits[2, "index"]))



# CI & treatment effect estimation
ci.data.wt200d <- subData.nomiss.wt200d.sbgrpWt.resPop.scaled[ci.ind.wt200d, ]
dim(ci.data.wt200d)
...
summ.eff.wt200d.nodes <- data.frame(node = rep(c("all", "node2", "node3"), each = 4),
                                    time = rep(1:4, 3),
...
save(df.eff.wt200d.nodes, summ.eff.wt200d.nodes, 
     ...
     file = "../../../Data/Data20210512/Processed/WtSubAnal200dAllStp5SbgrpTrjctr_20230410.RData")
...
# NEED TO LOOK NUMBER OF MALE AT TIME 4 AMONG TREATED
...
load("../../../Data/Processed/WtSubAnal200dAllStp5SbgrpTrjctr_20230410.RData")
...
p1 <- ggplot(summ.forFig %>% filter(node %in% c("node2", "node3")), aes(x = days, y = eff, group = node, color = node)) + 
  ...
  labs(x = "Day", 
       ...
       title = "Effect with bootstrap-based CI") + 
...
summ.ic.var <- data.frame(node = rep(c("node2", "node3"), each = 4),
                          days = rep(seq(200, 800, 200), 2),
                          eff  = summ.forFig$eff[summ.forFig$node != "all"],
                          q0025 = c(summ.wt200d.node2.t1$effect.measures$ATE$CI[1], 
                                    summ.wt200d.node2.t2$effect.measures$ATE$CI[1],
                                    summ.wt200d.node2.t3$effect.measures$ATE$CI[1],
                                    summ.wt200d.node2.t4$effect.measures$ATE$CI[1],
                                    summ.wt200d.node3.t1$effect.measures$ATE$CI[1], 
                                    summ.wt200d.node3.t2$effect.measures$ATE$CI[1],
                                    summ.wt200d.node3.t3$effect.measures$ATE$CI[1],
                                    summ.wt200d.node3.t4$effect.measures$ATE$CI[1]),
                          q0975 = c(summ.wt200d.node2.t1$effect.measures$ATE$CI[2], 
                                    summ.wt200d.node2.t2$effect.measures$ATE$CI[2],
                                    summ.wt200d.node2.t3$effect.measures$ATE$CI[2],
                                    summ.wt200d.node2.t4$effect.measures$ATE$CI[2],
                                    summ.wt200d.node3.t1$effect.measures$ATE$CI[2], 
                                    summ.wt200d.node3.t2$effect.measures$ATE$CI[2],
                                    summ.wt200d.node3.t3$effect.measures$ATE$CI[2],
                                    summ.wt200d.node3.t4$effect.measures$ATE$CI[2]))

p2 <- ggplot(summ.ic.var, aes(x = days, y = eff, group = node, color = node)) + 
  geom_point(position = position_dodge(width = 30)) + 
  geom_errorbar(aes(ymin = q0025, ymax = q0975), width = 30, position = position_dodge(width = 30)) +
  ylim(-1.1, 3.5) +
  labs(x = "Day", 
       y = "Estimated DTG effect (kg)", 
       title = "Effect with influence curve based CI") + 
  scale_color_manual(breaks = c("node2", "node3"),
                     labels = c("Male", "Female"),
                     values = c("black", "darkgray")) +
  theme_bw() + 
...
pdf("../../../Report/Manuscript20230402/WtSubAnal200dSbgrpTrjctr20230412.pdf", width = 4, height = 3.5)
# grid.arrange(p1, p2, nrow = 1)
p1
dev.off()

pdf("../../../Report/Manuscript20230402/WtSubAnal200dSbgrpTrjctr20230422.pdf", width = 7, height = 3.5)
grid.arrange(p1, p2, nrow = 1)
# p1
dev.off()
